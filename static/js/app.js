    ...
    computed: {
        seriesWithPills() {
            return this.series.map(s => {
                ...
                let finalPills = pillsToDisplay.reverse();

                let tmdbPill = null;
                if (s.tmdb_info && s.tmdb_info.total_episodes > 0) {
                    tmdbPill = {
                        text: `${s.downloaded_episodes_count} / ${s.tmdb_info.total_episodes}`,
    ...
    methods: {
        ...
        connectEventSource() {
            ...
            this.eventSource.onerror = (err) => {
                // Не логируем ошибку, если соединение закрыто намеренно или находится в процессе закрытия (2)
