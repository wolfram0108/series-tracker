const SettingsNamingQualityTab = {
  name: 'SettingsNamingQualityTab',
  template: `
    <div class="settings-tab-content">
        <div class="modern-fieldset mb-4">
            ...
            <div class="fieldset-content">
                <p class="text-muted small mb-3">Определите стандартизированные значения (например, '1080p WEB-DL') и поисковые паттерны для них. Используйте '*' для любого текста.</p>
                ...
                <div class="field-group">
                    <constructor-group>
                        ...
                        <div class="constructor-item item-floating-label">
                            <input type="text" class="item-input" id="quality-standard-new" placeholder=" " v-model="newPattern.standard_value" @keyup.enter="addPattern">
                            <label for="quality-standard-new">Новый стандарт качества, напр: 1080p WEB-DL</label>
                        </div>
                ...
                <div v-for="(qp, index) in patterns" :key="qp.id" class="list-card quality-standard-card mb-3">
                    <div class="list-card-header">
                        <div class="standard-title">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" role="switch" v-model="qp.is_active" @change="updatePattern(qp)">
                        ...
                        <div class="standard-controls">
                           ...
                           <button class="control-btn text-danger" @click="deletePattern(qp.id)" title="Удалить стандарт"><i class="bi bi-trash"></i></button>
                        </div>
                    </div>
                    <div class="list-card-body">
                        <div class="search-patterns-list mb-2" v-if="qp.search_patterns.length > 0">
                        ...
                        <constructor-group>
                            ...
                            <div class="constructor-item item-floating-label">
                                <input type="text" class="item-input" placeholder=" " v-model="qp.newSearchPattern" @keyup.enter="addSearchPattern(qp)" :ref="'patternInput' + qp.id">
                                <label>Новый поисковый паттерн...</label>
                            </div>
                            <div class="constructor-item item-button-group">
                                <button class="btn-icon btn-settings" @click="insertSymbol(qp, '*')" title="Вставить '*'">*</button>
                                <button class="btn-icon btn-add btn-text-icon" @click="addSearchPattern(qp)" :disabled="!qp.newSearchPattern">
                                    <i class="bi bi-plus-lg"></i>
                                </button>
                            </div>
                        </constructor-group>
                ...
                <div v-if="!patterns.length" class="empty-state">Стандартов качества пока нет.</div>
            </div>
        </div>
  ...
  emits: ['show-toast'],
  methods: {
    insertSymbol(qualityPattern, symbol) {
        const inputEl = this.$refs['patternInput' + qualityPattern.id][0];
        const start = inputEl.selectionStart;
        const end = inputEl.selectionEnd;
        let currentVal = qualityPattern.newSearchPattern || '';
        qualityPattern.newSearchPattern = currentVal.substring(0, start) + symbol + currentVal.substring(end);
        inputEl.focus();
    ...
    async addPattern() {
        try {
            const response = await fetch('/api/quality_patterns', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(this.newPattern) });
    ...
    async updatePattern(pattern) {
        try {
            await fetch(`/api/quality_patterns/${pattern.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(pattern) });
        } catch (error) { this.$emit('show-toast', error.message, 'danger'); }
    },
    ...
    async addSearchPattern(qualityPattern) {
        try {
    ...
    async testPatterns() {
        ...
        try {
            const response = await fetch('/api/quality_patterns/test', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename: this.testData.filename }) });
