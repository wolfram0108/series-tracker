const LogsModal = {
  ...
  computed: {
    ...
    // ИЗМЕНЕНИЕ: Вычисляемое свойство для определения, есть ли несохраненные изменения
    isLimitUnchanged() {
        // Сравниваем числовые значения, чтобы избежать проблем с типами (строка vs число)
  ...
  methods: {
    ...
    getLogRowClass(level) {
        switch(level) {
            case 'INFO': return 'table-info-light';
            case 'DEBUG': return 'table-debug-light';
            case 'WARNING': return 'row-warning-animated';
            case 'ERROR': return 'row-danger';
            default: return '';
        }
    },
