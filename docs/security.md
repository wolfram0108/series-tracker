...
- Хранение: логин + argon2-хэш пароля (таблица `admin_user`, владелец `auth`).
- **Онбординг первого запуска:** если админа нет — UI показывает модалку
  «Первый запуск» (имя/пароль/повтор, мин. 8) вместо входа; `POST /api/setup`
  создаёт админа и сразу логинит, активен только пока админа нет (после — 403).
  `.env`-bootstrap (`ST_ADMIN_USER`/`ST_ADMIN_PASSWORD`) — опция для headless.
- Эндпоинты: `POST /api/login`, `POST /api/logout`, `GET /api/me`,
  `POST /api/setup`, `GET /api/auth/status`.
...
- Фронт: модалки входа/установки в стиле проекта (StGroup/StInput,
  валидация рамкой, плашка-уведомление под окном). Визуал принят;
  эффект «матового стекла» — решено НЕ делать (текущий вид финальный).
...
- [x] `/docs` закрыт; parse_url ходит только на известные трекеры (SSRF
  митигирован); опасные ручки — под замком при включённом входе. (Этап 4)
- [x] Наружу не уходят стек-трейсы (generic-обработчик ошибок). (Этап 4)
