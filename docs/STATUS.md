...
VU11 был дырой и починен — `ignored_seasons` теперь применяется в плане.
Торрент-аналог игнор-сезонов (U12) — тем же путём НЕ покрыт: торрент-план
идёт через `_replace_plan`, не через `build_plan`.)
