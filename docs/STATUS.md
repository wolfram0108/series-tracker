...
(Формальная сверка `*_match_production` неактуальна — старой системы нет;
`torrents._drop_task` рантайм-деактивация уже сделана в R3; VU9 верифицирован;
VU11 был дырой и починен — `ignored_seasons` применяется в плане. Игнор-сезоны
— **VK-функция** (задаётся только в VK-композиции); у торрентов такого UI и
посезонной модели плана нет — U12 неприменим, это не дыра.)
...
коде и чиним дыры. **Обе части ЗАКРЫТЫ.** Торрент — 62 ✓, 5 ◐. VK —
46 ✓, 1 ✓◐, 6 ◐, 0 нерешённых.

Реальные дыры торрент-части:
...
Реальные дыры VK-части:
- **R4** — обычный скан не сверял диск VK (`downloads.fs.sync`): удалил .mp4 →
  скан не замечал и не перекачивал. Симметрично торрент `fs.verify` (`9967fa7`).
- **VF6** — `_remove_partial` чистил generic-паттерны yt-dlp, но не реальные
  артефакты этого кода (`.download.*`/`.remux.mp4`); при обрыве в фазе download
  они оставались мусором (`e1cbe9a`).
- **VP7/VL2/VL5** — `run_ffmpeg` писал в финал неатомарно: упавшая/убитая (kill -9)
  нарезка оставляла битый файл под финальным именем, и `_init_progress`
  усыновлял его как готовый эпизод. Теперь tmp→`os.replace` (находка 50) (`86a1909`).
- **VF2** — обычный скан не сверял нарезанных детей (`slicing.verify` звала только
  композиция, `fs.sync` компиляции пропускает); + F7-гард в `_verify_item` (`2372ed6`).

Остаточные ◐ (приемлемые/отложенные): VU9 (приоритет качества не заменяет уже
...
цикл, перекачки НЕТ), VC5 (save_path во время загрузки).
