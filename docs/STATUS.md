...
**Читать:** [contracts/series_situations.md](../contracts/series_situations.md) — полный
чек-лист всех ситуаций с сериалом + **раздел «ПЛАН ПРОДОЛЖЕНИЯ»** в конце
(точка возобновления, подробно).

Суть: переносим ЛОГИКУ (не код), каждую ситуацию симулируем на реальном
коде и чиним дыры. **Торрент-часть ЗАКРЫТА** (62 ✓, 5 ◐). Найдены и
починены реальные дыры:
- **R2** — re-add раздачи после удаления из qB не запускал конвейер (`84951c8`).
- **R1/R3/P10** — скан не сверял файлы; исчезнувшая раздача оставляла фантом
  `is_active`; застрявшая (пауза 0%, без задачи) не перезапускалась (`c2133cb`).
  Живой пруф: расстопорил «Путешествие».
- **Д1** — счётчик «скачано» не уезжал в SSE (`5c7e6bc`).
- **Д2** — удаление серии не прерывало in-flight yt-dlp/ffmpeg/перемещение
  и не сносило полуфайлы (`afe06dc`,`58bf7b5`). Принцип: **удаление НИКОГДА
  не трогает законченные файлы**.
- **F7** — при недоступном NAS `fs.verify` помечал всё ложно missing (`29ec2c8`).
- **C9** — конвейерное переименование шло без per-series лока (гонка с
  reprocess) — **вероятный корень кривых имён `.mkv.mkv.mkv`** (`27d7a71`).

**Следующее (см. план в матрице):** VK-часть (VU/VF/VS/VP/VL/VC); главная
известная дыра там — **R4** (скан не зовёт `downloads.fs.sync` → пропажа
VK-файла не детектится на скане; чинить в `scan/module.py::_run_series`
VK-ветка). Отложено: VP3 (yt-dlp «96% без файла»).
