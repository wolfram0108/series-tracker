# Прогресс переписывания фронта (журнал)

> ⚙️ **Исторический документ** — журнал переписывания фронта. Фронт **готов
> и выпущен** (v2.0.0); это справочный материал, не «в процессе».
...
| **Ф4** — перенос экранов на живые данные | ✅ **закрыт** (принято пользователем; все экраны §4) |
| Ф5 — приёмка паритета + e2e | ✅ **закрыт** (инварианты+визуалка+13 e2e) |
| Ф6 — cutover `/v2`→`/` | ✅ **ЗАКРЫТ** — новый фронт на `/`, старый на `/legacy`, `/v2`→307 |

**ПЕРЕПИСЫВАНИЕ ФРОНТА ЗАВЕРШЕНО (Ф0–Ф6).** Корень `/` = новый фронт
(Vite/Vue3/TS/PrimeVue). Старый — `/legacy` (откат). `/v2` → 307 на `/`.
Деплой: `git pull` → `npm run build` (в `web/`) → `chown -R user:user web` →
`sudo systemctl restart series-tracker` (рестарт нужен из-за python-правок
в gateway; для одной статики — не нужен). Откат: вернуть `GET /` на старый
index (или просто `/legacy`).
...
4. ~~Отладка-доп + DatabaseViewer~~ ✅ собран (флаги/числа конвейера, просмотр/
   очистка БД; запись флага доказана round-trip UI↔API). Принято.
5. ~~Логи: вкладка «Настройка»~~ ✅ собрана (была пропущена в Ф4 — у старой
   модалки логов 2 вкладки: «Просмотр» + «Настройка»). `LogsLoggingTab`:
   детализация логов по группам модулей (мастер-тумблер на группу) + флаги
   сохранения сырых файлов отладки. Контракт `debug_flags` (logging_modules +
   file_dump_flags), доказан round-trip групповым тумблером. ИСПРАВЛЕНИЕ ранее
   ошибочной заметки: старый UI ЭТО ПОКАЗЫВАЛ (вкладка логов), а не «не показывал».
6. **Ф4 закрыт.** Дальше Ф5 (приёмка паритета + e2e Playwright) → Ф6 (cutover).

**Ф5 — инварианты ✅ ПРОВЕРЕНЫ ЭМПИРИЧЕСКИ (сетевой прогон Playwright):**
- одно SSE-соединение: `/api/stream` = 1;
- viewing-stop: открытие статуса → POST `{state:["viewing"]}`, закрытие →
  POST `{state:[]}` (оба тела пойманы);
- частичный merge series_updated: `GET /api/series` = 1 за сессию (открытие
  статуса дёрнуло series_updated, список НЕ перезапрошен; код — `applyDelta`
  Object.assign на месте);
- 15-мин scan/AbortController: МООТ — клиентского интервала нет ни в новом,
  ни в старом фронте (grep пуст); скан серверный (модуль scanner), фронт
  слушает SSE. Добавлять нечего.
**Ф5 — визуальная сверка ✅ ПРОЙДЕНА** (скриншоты /v2 на живых данных): главный
(4 карточки), Add, статус торрент (Свойства все поля/Композиция/История) и VK
(Свойства/Композиция 85 элементов+DnD приоритет/Нарезка пустое состояние «нет
компиляций» — верно для одиночных эпизодов/История), конфигуратор «Фильтры VK»
(3 шага), Настройки (Авторизация/Трекеры/Агенты/Отладка с флагами+БД), Логи
(Просмотр+Настройка), просмотр БД. Всё рендерится корректно.

**Ф5 — e2e-сетка ✅ (13 тестов, все зелёные).** Playwright-раннер добавлен
(`@playwright/test`, `web/playwright.config.ts`, firefox, workers:1, против
живого :5000). Специи `web/e2e/`: main (список+инварианты SSE/series GET),
status (торрент-вкладки, VK-Нарезка, viewing-stop round-trip), settings
(вкладки, флаг round-trip, просмотр БД), configurator (профиль→правила→блоки,
CRUD профиля создать→удалить, тест-прогон→карточка исключения), logs (дефолты
фильтров, Настройка + тумблер группы round-trip), add (валидация+отмена).
Запуск: `npm run test:e2e`. **Безопасность:** мутации только обратимые
(создать→удалить, тумблер on→off, viewing open→close); afterAll чистит
e2e-tmp-профили; add — без реального POST. Стенд остаётся в исходном состоянии.

**Ф5 ЗАКРЫТ.** Дальше — Ф6 (cutover `/v2`→`/`).
