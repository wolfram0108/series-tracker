    ...
    def _process_mass_torrent_task(self, task):
        ...
        with self.app.app_context():
            ...
            for torrent_db_entry in active_torrents:
                ...
                # Вызываем нашу единую функцию для каждого торрента с распределением по сезонам
                process_and_rename_torrent_files(self.app, series_id, qb_hash)
        
            # Метод для переименования всех существующих папок сезонов больше не используется
            # так как переименование теперь происходит через основную функцию с распределением по сезонам